...
function generateExtentReport(reportFolder, evidenceData, meta) {
    ...
    if (!evidenceData || evidenceData.length === 0) {
        console.log('⚠️  No evidence data — generating empty extent report shell');
        evidenceData = [];
    }
    ...
    if (evidenceData.length === 0) {
        console.log('⚠️  No valid API data after sanitisation — generating empty extent report shell');
    }
...
html{zoom:100%;-webkit-text-size-adjust:100%;text-size-adjust:100%;}
...
function toggleOvRow(idx) {
    var rows = document.querySelectorAll('.ov-api-row[data-iter="'+idx+'"]');
    var arr  = document.getElementById('ova-'+idx);
    // check current state from first matching row
